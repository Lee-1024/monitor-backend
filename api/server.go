...
func (s *APIServer) setupRoutes() {
	...
	}
	{
		...
		// 运维助手路由
		opsAssistant := v1.Group("/ops-assistant")
		{
			opsAssistant.POST("/chat", s.chatOpsAssistant)
			opsAssistant.GET("/chat/stream", s.streamOpsAssistant)
			opsAssistant.GET("/sessions", s.listOpsAssistantSessions)
			opsAssistant.GET("/sessions/:id", s.getOpsAssistantSession)
			opsAssistant.DELETE("/sessions/:id", s.deleteOpsAssistantSession)
		}
